...
local checkpoints = {
    Basecamp = CFrame.new(-242, 37.2616577, 188),
    CP1 = CFrame.new(-420, 191.542892, 556),
    CP2 = CFrame.new(-624, 579.568237, 1135),
    CP3 = CFrame.new(-902, 689.690308, 1225),
    CP4 = CFrame.new(-386, 1041.2616, 2047),
    CP5 = CFrame.new(-755, 1525.2616, 2057),
    CP6 = CFrame.new(-475, 1633.2616, 2283),
    CP7 = CFrame.new(-87, 1929.2616, 2117),
    CP8 = CFrame.new(1, 2493.26147, 2045)
}

local checkpointOrder = {"Basecamp", "CP1", "CP2", "CP3", "CP4", "CP5", "CP6", "CP7", "CP8"}
local checkpointSummit = {"CP1", "CP2", "CP3", "CP4", "CP5", "CP6", "CP7", "CP8"}
...
local function summitLoop()
    while state.running do
        ...
        for i, cpName in ipairs(checkpointSummit) do
            ...
            if cframe then
                ...
                if carriedChar then
                    local carriedCFrame = cframe + Vector3.new(0, 0, 3)
                    teleportCharacter(carriedChar, CFrame.new(carriedCFrame))
                end
                
                task.wait(state.summitDelay)
            end
        end
...
local Window = Rayfield:CreateWindow({
    Name = "🛡️ ShieldTeam | NERO Ultimate v2.1",
...
AutoTab:CreateInput({
    Name = "Set Summit Delay (seconds)",
    PlaceholderText = "e.g. 3",
    CurrentValue = "3",
    Callback = function(val)
        local delay = tonumber(val)
        if delay and delay >= 0.1 then
            state.summitDelay = delay
            Rayfield:Notify({
                Title="Delay Updated", 
                Content="Summit delay set to " .. delay .. " seconds", 
                Duration=2
            })
        else
            Rayfield:Notify({
                Title="Invalid Delay", 
                Content="Please enter a number >= 0.1", 
                Duration=2
            })
        end
    end
...
AutoTab:CreateToggle({
    Name = "Auto Loop Summit (Basecamp → CP8)",
    CurrentValue = false,
    Callback = function(val)
        state.running = val
        if val then 
            ...
            Rayfield:Notify({
                ...
                Content="Auto summit started with carry support (Delay: " .. state.summitDelay .. "s)", 
                Duration=3
            })
        else
            Rayfield:Notify({
...
AutoTab:CreateButton({
    ...
    Callback = function()
        if not state.running then
            ...
            Rayfield:Notify({
                Title="Summit", 
                Content="Running summit once (Delay: " .. state.summitDelay .. "s)", 
                Duration=2
            })
        else
            Rayfield:Notify({
                Title="Summit Running", 
                Content="Auto summit is already running", 
                Duration=2
            })
        end
    end
...
InfoTab:CreateParagraph({
    ...
    Content = "Auto Summit (Basecamp→CP8) with customizable delay • Carry player support • InfinityYield-style fly • Anti-reset speed hack • Enhanced player teleport system • ESP & Noclip • Fake admin title"
})

InfoTab:CreateParagraph({
    Title = "🏔️ Summit Route",
    Content = "Basecamp → CP1 → CP2 → CP3 → CP4 → CP5 → CP6 → CP7 → CP8 (loops continuously with carry support)"
})
